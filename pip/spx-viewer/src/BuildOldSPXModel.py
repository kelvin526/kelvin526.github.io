...
def get_spx_data(_currentJsonContent: str, Today, Yesterday) -> str:
	...
	for x in range(1):
		...
		for y in TodaySet:
			if (counter == 3) :
				...
				z = 0
			Today = datetime.datetime(TodayYear + z, TodaySet[counter], 1, 0, 0, 0, 0, tzinfo=datetime.timezone.utc) 
			Yesterday = datetime.datetime(TodayYear, YesterdaySet[counter], 1, 0, 0, 0, 0, tzinfo=datetime.timezone.utc)
			filename = f"model/{Yesterday.year}.json"
			currentJsonContent = ""
			try:
			...
			currentJsonContent = get_spx_data(currentJsonContent, Today, Yesterday)
